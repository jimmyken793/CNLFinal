...
public class TOTorrentDeserialiseImpl extends TOTorrentImpl {
	...
	throws TOTorrentException {
		...
		// parg: there used to be a check made that the torrent file wasn't
		// larger than 1MB.
	...
	throws TOTorrentException {
		...
		try {
			byte[] buf = new byte[32 * 1024]; // raised this limit as 2k was
												// rather too small
			...
			if (iFirstByte != 'd' && iFirstByte != 'e' && iFirstByte != 'i' && !(iFirstByte >= '0' && iFirstByte <= '9')) {
				...
				// often people download an HTML file by accident - if it looks
				// like HTML
		...
		try {
			...
			while (root_it.hasNext()) {
				...
				if (key.equalsIgnoreCase(TK_ANNOUNCE)) {
					...
					if (ann_list instanceof List) { // some malformed torrents
													// have this key as a
													// zero-sized string instead
													// of a zero-sized list
					...
					if (announce_list != null && announce_list.size() > 0) {
						...
						for (int i = 0; i < announce_list.size(); i++) {
							...
							if (temp instanceof List) {
								...
								for (int j = 0; j < set.size(); j++) {
									...
									// check to see if the announce url is
									// somewhere in the announce-list
						...
						// if the original announce url isn't found, add it to
						// the list
						// watch out for those invalid torrents with announce
						// url missing
				...
				for (int i = 0; i < files.length; i++) {
					...
					while (file_it.hasNext()) {
						...
						if (key.equals(TK_LENGTH) || key.equals(TK_PATH)) {
							...
							// standard
							// we don't skip TK_PATH_UTF8 because some code
							// might assume getAdditionalProperty can get it
						} else {
