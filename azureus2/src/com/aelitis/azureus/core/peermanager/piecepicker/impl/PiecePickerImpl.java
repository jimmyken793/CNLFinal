...
public class PiecePickerImpl implements PiecePicker {
	...
	/**
	 * min ms for recalculating availability - reducing this has serious
	 * ramifications
	 */
	...
	// The following are added to the base User setting based priorities (for
	// all inspected pieces)
	...
	private static final int PRIORITY_W_FILE_BASE = 1000; // min amount added
															// for priority
															// file. >
															// first/last piece
															// priority offset
															...
															// over f/l piece
															// priority
	private static final int PRIORITY_W_FILE_RANGE = 1000; // priority range
															// added for
															// prioritized files
	...
	/**
	 * currently webseeds + other explicit priorities are around 10000 or more -
	 * at this point we ignore rarity
	 */
	...
	/**
	 * Default number of requests sent to a peer, (for each X B/s another
	...
	/**
	 * The rarest availability level of pieces that we affirmatively want to try
	 * to request from others soonest ie; our prime targets for requesting
	 * rarest pieces
	 */
	...
	/**
	 * The list of chunks needing to be downloaded (the mechanism change when
		...
		class ParameterListenerImpl implements ParameterListener {
			...
			public final void parameterChanged(final String parameterName) {
				if (parameterName.equals("Prioritize Most Completed Files")) {
					...
					paramPriorityChange++; // this is a user's priority change
											// event
	...
	public PiecePickerImpl(final PEPeerControl pc) {
		...
		// initialize each piece; on going changes will use event driven
		// tracking
	...
	public final void addHavePiece(final PEPeer peer, final int pieceNumber) {
		...
		// if this is an interesting piece then clear any record of
		// "no requests" so the peer gets
		// scheduled next loop
	...
	/**
	 * This methd will compute the pieces' overall availability (including
	 * ourself) and the _globalMinOthers & _globalAvail
	 */
	@Override
	public final void updateAvailability() {
			...
			try {
				...
				if (Constants.isCVSVersion()) {
					...
					if (errors > 0 && errors != nbPieces) {
						if (Logger.isEnabled()) {
							Logger.log(new LogEvent(peerControl, LOGID, LogEvent.LT_ERROR, "updateAvailability(): availability rebuild errors = " + errors + " timeAvailRebuild =" + timeAvailRebuild));
						}
		...
		for (i = 0; i < nbPieces; i++) {
			...
			if (avail > 0 && avail < rarestMin && dmPiece.isDownloadable() && (pePiece == null || pePiece.isRequestable())) {
				rarestMin = avail; // most important targets for near future
	...
	/**
	 * Early-outs when finds a downloadable piece Either way sets
	 * hasNeededUndonePiece and neededUndonePieceChange if necessary
	 */
	...
	/**
	 * one reason requests don't stem from the individual peers is so the
	 * connections can be sorted by best uploaders, providing some ooprtunity to
	 * download the most important (ie; rarest and/or highest priority) pieces
	...
	public final void allocateRequests() {
		...
		for (int i = 0; i < peersSize; i++) {
			...
			if (peer.isDownloadPossible()) {
				...
				if (no_req_count == 0 || allocate_request_loop_count % (no_req_count + 1) == 0) {
					...
					// final long upRate
					// =peer.getStats().getSmoothDataReceiveRate();
					// UnchokerUtil.updateLargestValueFirstSort(upRate, upRates,
		...
		/*
		 * sort all peers we're currently downloading from with the most
		 * favorable for the next request one as 1st entry randomize list first
		 * to not pick the same candidates if the list of best doesn't return
		 * conclusive results
		 */
		...
		Collections.sort(bestUploaders, new Comparator() {
			...
			public int compare(Object o1, Object o2) {
				...
				/*
				 * pt1 comes first if we want to request data from it more than
				 * from pt2 it is "smaller", i.e. return is < 0
				 */
				int toReturn = 0;

				// lan peers to the front of the queue as they'll ignore request
				// limiting
				...
				// try to download from the currently fastest, this is important
				// for the request focusing
				if (toReturn == 0) {
					toReturn = (int) (stats2.getSmoothDataReceiveRate() - stats1.getSmoothDataReceiveRate());
				}

				// we're here because we're not requesting anything from both
				// peers
				// which means we might have to send the next request to this
				// peer

				// first try to download from peers that we're uploading to,
				// that should stabilize tit-for-tat a bit
				...
				// try some peer we haven't downloaded from yet (this should
				// allow us to taste all peers)
				...
				/*
				 * 
				 * // still nothing, next try peers from which we have
				 * downloaded most in the past // NO, we don't want to focus on
				 * what may have become a bad peer if(toReturn == 0) toReturn =
				 * (int)(stats2.getTotalDataBytesReceived() -
		...
		if (priorityRTAexists) {
			...
			// to keep the ordering consistent we need to use a fixed metric
			// unless
			// we remove + re-add a peer, at which point we need to take account
			// of
					...
					if (result == 0) {
						...
						if (result == 0) {
							...
							if (result == 0) {
								...
								// v unlikely - inconsistent but better than
								// losing a peer
			...
			// we need to sort by how quickly the peer can get a block, not just
			// its base speed
			...
			try {
				while (allocated_request && priorityRTAexists) {
					...
					while (!block_time_order_peers.isEmpty()) {
						...
						// ignore request number advice from peers in RTA mode,
						...
						if (allowed_requests > 0) {
							...
							if (!done_priorities) {
								...
								if (!priorityRTAexists) {
									...
									// might have stopped RTA as this is
									// calculated in computeBasePriorities
							...
							if (findRTAPieceToDownload(pt, pt == best_uploader, best_block_eta)) {
								...
								// add back in to see if we can allocate a
								// further request
			...
			// if we have plenty of blocks outstanding we can afford to be more
			// generous in the
		...
		for (int i = 0; i < uploadersSize; i++) {
			...
			// only request when there are still free tokens in the bucket or
			// when it's a lan peer (which get sorted to the front of the queue)
			...
			if (pt.isDownloadPossible()) {
				...
				// Only loop when 3/5 of the queue is empty, in order to make
				...
				if (pt.getNbRequests() <= (maxRequests * 3) / 5) {
					...
					if (total_allocated == 0) {
						...
						// there are various reasons that we might not allocate
						// any requests to a peer
						// such as them not having any pieces we're interested
						// in. Keep track of the
						// number of consecutive "no requests" outcomes so we
						// can reduce the scheduling
						...
						// System.out.println( pt.getIp() + ": nb=" +
						// pt.getNbRequests() + ",max=" + maxRequests + ",nrc="
						// + no_req_count +",loop=" +
						// allocate_request_loop_count);
	...
	/**
	 * Count current global min avail pieces in progress (not counting
	 * non-rarest pieces but keep them to compensate high churn, remove
	 * completed ones, ignore idle ones)
	 * 
	 * @return number of pieces that may be started due to the "rarest first"
	 *         picking rule
	 */
	private int calcRarestAllowed() {
		...
		if (SystemTime.getCurrentTime() - peerControl.getTimeStarted() < 180 * 1000) {
			RarestAllowed = 0;
		}
		...
		for (int i = 0; i < rarestStartedPieces.size(); i++) {
			...
			if ((rarestStarted.getAvailability() <= globalMinOthers || globalMinOthers > globalMin) && (SystemTime.getCurrentTime() - rarestStarted.getLastDownloadTime(SystemTime.getCurrentTime()) < 60 * 1000 || rarestStarted.getNbWritten() == 0) && !rarestStarted.isDownloaded()) {
				RarestAllowed--;
			}
		}
	...
	/**
	 * This computes the base priority for all pieces that need requesting if
	 * there's been any availability change or user priority setting changes
	 * since the last call, which will be most of the time since availability
	 * changes so dynamicaly It will change startPriorities[] (unless there was
	 * nothing to do)
	 */
	private final void computeBasePriorities() {
		...
		if (!priorityRTAexists) {
			if (startPriorities != null && ((now > timeLastPriorities && now < time_last_avail + TIME_MIN_PRIORITIES) || (priorityParamChange >= paramPriorityChange && priorityFileChange >= filePriorityChange && priorityAvailChange >= availabilityChange))) {
				return; // *somehow* nothing changed, so nothing to do
			}
		}

		// store the latest change indicators before we start making dependent
		// calculations so that a
		...
		try {
			...
			// calculate all base (starting) priorities for all pieces needing
			// requesting
			...
			for (int i = 0; i < nbPieces; i++) {
				...
				for (int j = 0; j < pieceListSize; j++) {
					...
					if (length > 0 && downloaded < length && !fileInfo.isSkipped()) {
						...
						if (firstPiecePriorityL && fileInfo.getNbPieces() > FIRST_PIECE_MIN_NB) {
							/*
							 * backed out for the moment - reverting to old
							 * first/last piece only int lastFirstPiece =
							 * fileInfo.getFirstPieceNumber() +
							 * FIRST_PIECE_RANGE_PERCENT *
							 * (fileInfo.getLastPieceNumber() -
							 * fileInfo.getFirstPieceNumber()) / 100;
							 * 
							 * if ( (i >=fileInfo.getFirstPieceNumber() && i<=
							 * lastFirstPiece ) ) { priority
							 * +=PRIORITY_W_FIRSTLAST + 10 * (lastFirstPiece -
							 * i) ; }
							 * 
				...
				if (startPriority >= 0) {
					...
					final int avail = availability[i];
					// nbconnects is async calculate so may be wrong - make sure
					// we don't decrease pri by accident
					if (avail > 0 && nbConnects > avail) { // boost priority for
															// rarity
						startPriority += nbConnects - avail;
						// startPriority +=(PRIORITY_W_RARE
						// +peerControl.getNbPeers()) /avail;
						// Boost priority even a little more if it's a globally
						// rarest piece
	...
	private final boolean isRarestOverride() {
		...
		// Dont seek rarest under a few circumstances, so that other factors
		// work better
		...
		if (!rarestOverride && nbRarestActive > 1 && globalMinOthers > 1) {
			// if already getting some rarest, dont get more if swarm is healthy
			// or too many pieces running
			rarestOverride = globalMinOthers > globalMin || (globalMinOthers >= (2 * nbSeeds) && (2 * globalMinOthers) >= nbPeers);
			// Interest in Rarest pieces (compared to user priority settings)
			// could be influenced by several factors;
			// less demand closer to 0% and 100% of torrent completion/farther
			// from 50% of torrent completion
			// less demand closer to 0% and 100% of peers interestd in
			// us/farther from 50% of peers interested in us
			// less demand the more pieces are in progress (compared to swarm
			// size)
			// less demand the farther ahead from absolute global minimum we're
			// at already
			// less demand the healthier a swarm is (rarity compared to # seeds
			// and # peers)
		}
	...
	protected final int findPieceToDownload(PEPeerTransport pt, int nbWanted) {
		...
		if (pieceNumber < 0) {
			// probaly should have found something since chose to try; probably
			// not interested anymore
			// (or maybe Needed but not Done pieces are otherwise not
			// requestable)
			// pt.checkInterested();
			return 0;
		}
			...
			for (int i = startI; i <= endI; i++) {
				...
				if (peerHavePieces.flags[i] && startPriorities[i] == PRIORITY_REALTIME && piece_rta > 0) {
					...
					if (piece_rta >= piece_min_rta_time) {
						...
						// only allocate if we have a chance of getting this
						// block in time or we're
						// the best uploader we've got/even the best uploader
						// can't get it

						// the second part is important for when we get to the
						// point whereby no peers
						// can get a block in time. Here we need to allocate
						// someone to get it as
						// otherwise we'll concentrate on getting lower priority
						// pieces that we can
						// get in time and leave the stuck ones for just the
						// best uploader to get
						...
						// check the blocks to see if any are now lagging behind
						// their ETA given current peer speed
						...
						for (int j = 0; j < peer_requests.length; j++) {
							...
							if (!pt_already_present) {
								...
								// and there are no outstanding requests or
								// outstanding requests are lagging
								...
								if (block_peer_requests.size() == 0) {
									...
									break; // earlier blocks always have
											// priority

								} else if (best_eta > piece_rta && (best_uploader || !try_allocate_even_though_late)) {

									// don't re-allocate when we're already
									// running late as we'll end up
									...
									// if we can do better than existing best
									// effort allocate
	...
	/**
	 * This method is the downloading core. It decides, for a given peer, which
	 * block should be requested. Here is the overall algorithm : 0. If there a
	 * FORCED_PIECE or reserved piece, that will be started/resumed if possible
	 * 1. Scan all the active pieces and find the rarest piece (and highest
	 * priority among equally rarest) that can possibly be continued by this
	 * peer, if any 2. While scanning the active pieces, develop a list of
	 * equally highest priority pieces (and equally rarest among those) as
	 * candidates for starting a new piece 3. If it can't find any piece, this
	 * means all pieces are already downloaded/full requested 4. Returns int[]
	 * pieceNumber, blockNumber if a request to be made is found, or null if
	 * none could be found
	 * 
	 * @param pc
	 *            PEPeerTransport to work with
	 * 
	 * @return int with pieceNumberto be requested or -1 if no request could be
	 *         found
	 */
	private final int getRequestCandidate(final PEPeerTransport pt) {
		...
		// If there's a piece seserved to this peer resume it and only it (if
		// possible)
		...
		if (reservedPieceNumbers != null) {
			...
			// note, pieces reserved to peers that get disconnected are released
			// in pepeercontrol
		}
		...
		boolean resumeIsRarest = false; // can the peer continuea piece with
										// lowest avail of all pieces we want
		...
		int priority; // aggregate priority of piece under inspection (start
						// priority or resume priority for pieces to be resumed)
		int avail = 0; // the swarm-wide availability level of the piece under
						// inspection
		long pieceAge; // how long since the PEPiece first started downloading
						// (requesting, actually)
		...
		for (i = startI; i <= endI; i++) {
			...
			if (peerHavePieces.flags[i]) {
				...
				if (priority >= 0 && dmPiece.isDownloadable()) {
					...
					// if we are considering realtime pieces then don't bother
					// with non-realtime ones

					if (pePiece == null || pePiece.isRequestable()) {
						// if this priority exceeds the priority-override
						// threshold then we override rarity
						boolean pieceRarestOverride = priority >= PRIORITY_OVERRIDES_RAREST ? true : rarestAllowed;

						// piece is: Needed, not fully: Requested, Downloaded,
						// Written, hash-Checking or Done

						avail = availability[i];
						if (avail == 0) { // maybe we didn't know we could get
											// it before
							availability[i] = 1; // but the peer says s/he has
													// it
							avail = 1;
						}
						...
						if (pePiece != null) {
							if (priority != startPriorities[i]) {
								pePiece.setResumePriority(priority); // maintained
																		// for
																		// display
																		// purposes
																		// only
							...
							// How many requests can still be made on this
							// piece?
							...
							if (peerReserved != null) {
								...
								// the peer forgot this is reserved to him;
								// re-associate it
								pt.addReservedPieceNumber(i);
								return i;
							}
							...
							if (pt.isSnubbed()) {
								// snubbed peers shouldn't stall fast pieces
								// under ANY condition
								// may lead to trouble when the snubbed peer is
								// the only seed, needs further testing
								mayResume &= pieceSpeed < 1;
								mayResume &= freeReqs > 2 || avail <= nbSnubbed;
							} else {
								// slower peers are allowed as long as there is
								// enough free room
								mayResume &= freeReqs * peerSpeed >= pieceSpeed / 2; // ||
																						// rarestPrio;
								// prevent non-subbed peers from resuming on
								// snubbed-peer-pieces but still allow them to
								// resume stalled pieces
								mayResume &= peerSpeed < 2 || pieceSpeed > 0 || pePiece.getNbRequests() == 0;
								mayResume |= i == pt.getLastPiece();
							}

							// find a fallback piece in case the peer could
							// theoretically contribute
							// to an existing piece but is prevented by the
							// snubbing rules etc.
							...
							pePiece.setResumePriority(priority); // this is only
																	// for
																	// display

							if (avail < resumeMinAvail || (avail == resumeMinAvail && priority > resumeMaxPriority)) { // this
																														// piece
																														// seems
																														// like
																														// best
																														...
																														// this
																														// piece
								if (pePiece.hasUnrequestedBlock()) { // change
																		// the
																		// different
																		// variables
																		// to
						...
						} else if (avail <= globalMinOthers && rarestAllowed) { // rarest
																				// pieces
																				// only
																				// from
																				// now
							...
							if (!startIsRarest) { // 1st rarest piece
								...
								startCandidates.setOnly(i); // clear the
															// non-rarest bits
															// in favor of only
															// rarest
							} else if (priority > startMaxPriority) { // continuing
																		// rarest,
																		// higher
																		// priority
																		// level
						...
						} else if (!startIsRarest || !rarestAllowed) { // not
																		// doing
																		// rarest
																		// pieces
							if (priority > startMaxPriority) { // new priority
																// level
								if (startCandidates == null) {
									startCandidates = new BitFlags(nbPieces);
								}
								...
								if (avail < startMinAvail) { // same priority,
																// new
																// availability
																// level
									startMinAvail = avail;
									startIsRarest = avail <= globalMinOthers;
									startCandidates.setOnly(i);
								} else if (avail == startMinAvail) { // same
																		// priority
																		// level,
																		// same
																		// availability
		...
		/*
		 * // don't start pieces when snubbed, unless it's the only peer with
		 * that piece // returns -1 if no piece to resume is found
		...
		if (!forceStart || startCandidates == null || startCandidates.nbSet <= 0) {
			...
			// this would allow more non-rarest pieces to be resumed so they get
			// completed so they can be re-shared,
			// which can make us intersting to more peers, and generally improve
			// the speed of the swarm,
			// however, it can sometimes be hard to get the rarest pieces, such
			// as when a holder unchokes very infrequently
			// 20060312[MjrTom] this can lead to TOO many active pieces, so do
			// the extra check with arbitrary # of active pieces
			final boolean resumeIsBetter;
			if (reservedPieceNumber >= 0 && globalMinOthers > 0 && peerControl.getNbActivePieces() > 32) // check
																											// at
																											// arbitrary
																											// figure
																											// of
	 ...
	 * @return int the piece number that was chosen to be started. Note it's
	 *         possible for the chosen piece to have been started already (by
	 *         another thread). This method considers that potential to not be
	 *         relevant.
	 */
	...
	private final void checkEndGameMode() {
		...
		for (int i = 0; i < nbPieces; i++) {
			...
			final DiskManagerPiece dmPiece = dmPieces[i];
			// If the piece isn't even Needed, or doesn't need more downloading,
			// simply continue
			if (!dmPiece.isDownloadable()) {
				continue;
			}
			...
			// If the piece is being downloaded (fully requested), count it and
			// continue
			...
			// Else, some piece is Needed, not downloaded/fully requested; this
			// isn't end game mode
			return;
		}

		// when doing RTA we don't want EGM to kick in too early as it interfers
		// with progressive
		// playback by increasing discard. So we use a smaller trigger value to
		// limit the impact
	...
	private final void computeEndGameModeChunks() {
		try {
			...
			for (int i = 0; i < nbPieces; i++) {
				...
				// Pieces not Needed or not needing more downloading are of no
				// interest
	...
	/**
	 * adds every block from the piece to the list of chuncks to be selected for
	 * egm requesting
	 * 
	 */
	@Override
	...
	protected final int findPieceInEndGameMode(final PEPeerTransport pt, final int wants) {
		...
		try {
			...
			if (nbChunks > 0) {
				...
				if (pt.isPieceAvailable(pieceNumber) && pePiece != null && (!pt.isSnubbed() || availability[pieceNumber] <= peerControl.getNbPeersSnubbed()) && pt.request(pieceNumber, chunk.getOffset(), chunk.getLength()) != null) {
			...
			// cleanup anyway and allow a proper re-entry into endgame mode if
			// neccessary
	...
	public void addRTAProvider(PieceRTAProvider provider) {
		...
		// we don't want end-game mode kicking in and screwing with the RTA
		// logic
		// at the end of the download. simplest way is to abandon it for this
		// download. if someone gives up RT later then the download will just
		// complete
		// without EGM
	...
	/**
	 * An instance of this listener is registered with peerControl Through this,
	 * we learn of peers joining and leaving and attach/detach listeners to them
	 */
	...
	private class PEPeerListenerImpl implements PEPeerListener {
		...
		public final void stateChanged(PEPeer peer, final int newState) {
			/*
			 * switch (newState) { case PEPeer.CONNECTING: return;
			 * 
			 * case PEPeer.HANDSHAKING: return;
			 * 
			 * case PEPeer.TRANSFERING: return;
			 * 
			 * case PEPeer.CLOSING: return;
			 * 
			 * case PEPeer.DISCONNECTED: return; }
			 */
		}
	...
	private class DiskManagerListenerImpl implements DiskManagerListener {
		...
		public final void filePriorityChanged(DiskManagerFileInfo file) {
			...
			// only need to re-calc Needed on file's pieces; priority is
			// calculated seperatly
			boolean foundPieceToDownload = false;
			// if didn't have anything to do before, now only need to check if
			// we need
